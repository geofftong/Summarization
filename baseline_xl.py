...
flags.DEFINE_string("best_model_dir", default="best_model_dir",
                    help="Path of best_model_dir.")
flags.DEFINE_integer("eval_every_steps",
                     default=500, help="eval_every_steps")
...
flags.DEFINE_integer("num_core_per_host", default=2,
...
flags.DEFINE_integer("beam_sizes", default=2,
                     help="beam_sizes")
flags.DEFINE_integer("max_extract_turns", default=3,
                     help="max_extract_turns")

flags.DEFINE_string("local_check", default="",
                    help="check point")
    ...
    for entry in input_data:
        for paragraph in entry["paragraphs"]:
            ...
            for qa in paragraph["qas"]:
                ...
                L = sorted([(extract_state_on_sents[i], s) for i, s in enumerate(paragraph_seg_text) if
                            int(extract_state_on_sents[i]) != 0], key=lambda x: x[0])
                question_texts = [qa["question"].split('[SEP]')[0]] + [i[1] for i in L]
                question_text = '[SEP]'.join(question_texts)
                for sent_idx, state in enumerate(extract_state_on_sents):
                    if int(state) == 0:
                        ...
                        extract_state.extend([state for _ in range(len(paragraph_seg_text[sent_idx]))])
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            for i in range(doc_span.length):
                ...
                p_mask.append(0)
                ori_index_of_token_start = tok_start_to_orig_index[split_token_index]
                # ori_index_of_token_end = tok_end_to_orig_index[split_token_index]
                # temp = paragraph_text[ori_index_of_token_start:ori_index_of_token_end+1]
                # assert all_extract_state[ori_index_of_token_start]==all_extract_state[ori_index_of_token_end]
                span_extract_state.append(all_extract_state[ori_index_of_token_start])
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    # squad_utils.find_all_best_thresh_v2(out_eval, all_predictions, exact_raw, f1_raw,scores_diff_json, qid_to_has_ans)
...
def input_fn_builder(input_glob, seq_length, is_training, drop_remainder,
                     num_hosts, num_threads=8, is_evaluating=False):
    ...
    name_to_features = {
        ...
        "extract_state": tf.FixedLenFeature([seq_length], tf.int64),
...
def get_model_fn():
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.EVAL:
            ...
            start_logits = outputs["start_log_probs"]
            end_logits = outputs["end_log_probs"]
            cls_logits = outputs['cls_logits']
            null_pre = cls_logits > 0
            null_acc = tf.metrics.accuracy(is_impossible, null_pre)
            start_pre = tf.argmax(start_logits, 1)
            end_pre = tf.argmax(end_logits, 1)
            start_acc = tf.metrics.accuracy(start_positions, start_pre)
            end_acc = tf.metrics.accuracy(end_positions, end_pre)
            acc = tf.metrics.accuracy(start_positions + end_positions, start_pre + end_pre)
            all_loss = tf.metrics.mean(total_loss)
            null_loss = tf.metrics.mean(regression_loss)
            pos_loss = tf.metrics.mean(position_loss)
            eval_metrics = {
                'start_acc': start_acc,
                'end_acc': end_acc,
                'acc': acc,
...
def preprocess():
    ...
    all_examples = read_squad_examples(FLAGS.train_file, is_training=True)
    all_examples.sort(key=lambda x: x.qas_id)
    split_flag = 0
    temp_dic = {}
    for i in all_examples:
        ss = i.qas_id.split('-')[1]
        temp_dic[ss] = temp_dic.get(ss, 0) + 1
        split_flag += 1
        if len(temp_dic) == 501:
            break
    train_examples = all_examples[split_flag:]
    ...
    tf.logging.info("Read examples from {}".format(FLAGS.eval_file))
    eval_examples = all_examples[:split_flag]
    # eval_examples = train_examples[FLAGS.proc_id::FLAGS.num_proc]

    # Pre-shuffle the input to avoid having to make a very large shuffle
    # buffer in the `input_fn`.
    random.shuffle(eval_examples)

    tf.logging.info("Write to {}".format(eval_rec_file))
    eval_writer = FeatureWriter(
        filename=eval_rec_file,
        is_training=True)
    convert_examples_to_features(
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        # for i in range(0, eval_epoches + 2):
        ...
        #             writer.write("\n")
        #
        #         if result["loss"] < best_eval_metric:
        #             best_eval_metric = result["loss"]
        #             os.system('mkdir {}'.format(FLAGS.best_model_dir))
        #             os.system('rm -rf {}/*'.format(FLAGS.best_model_dir))
        #             import glob
    ...
    if FLAGS.do_predict:
        ...
        for turn_idx in range(1, max_turn + 1):  # text != ""
            ...
            predict_examples = read_squad_examples(predict_file, is_training=False)
            read_time = time.time()
            print(
            ...
            predict_rec_file = os.path.join(
                FLAGS.output_dir,
                "{}.slen-{}.qlen-{}.predict.tf_record_{}".format(
                    spm_basename, FLAGS.max_seq_length, FLAGS.max_query_length, str(turn_idx)))
            predict_feature_file = os.path.join(
                FLAGS.output_dir,
                "{}.slen-{}.qlen-{}.predict.features.pkl_{}".format(
                    spm_basename, FLAGS.max_seq_length, FLAGS.max_query_length, str(turn_idx)))

            if False and tf.gfile.Exists(predict_rec_file) and tf.gfile.Exists(
                    predict_feature_file) and not FLAGS.overwrite_data:
                tf.logging.info("Loading predict features from {}".format(predict_feature_file))
                with tf.gfile.Open(predict_feature_file, 'rb') as fin:
                    predict_features = pickle.load(fin)
            else:
                predict_writer = FeatureWriter(filename=predict_rec_file, is_training=False)
                predict_features = []
                ...
                with tf.gfile.Open(predict_feature_file, 'wb') as fout:
                    pickle.dump(predict_features, fout)
            ...
            cur_results = []
            if FLAGS.local_check == None or FLAGS.local_check == '':
                check_point = None
            else:
                check_point = os.path.join(FLAGS.model_dir, FLAGS.local_check)
            tf.logging.info(check_point)
            ...
            ret = write_predictions(predict_examples, predict_features, cur_results,
            ...
            with open(os.path.join(FLAGS.output_dir, "predictions_results_" + str(turn_idx)), "w",
                      encoding='utf') as fw:
